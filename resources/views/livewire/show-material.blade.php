            ...
            <div class="dropdown CRM_dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right"
                    ...
                    @php
                        $link =asset($row->link);
                        if ($row->type=="SCORM"){
                            if (!empty($row->scorm_version) && !empty($row->link)){
                            $link =route('scorm.viewer',[$row->scorm_version,base64_encode($row->link)]);                                                             }
                        }elseif ($row->type=='XAPI'){
                              $link =route('xapi.viewer',base64_encode($row->link));
                        }
                    @endphp
