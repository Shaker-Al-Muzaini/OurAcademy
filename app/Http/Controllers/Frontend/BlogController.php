        ...
        try {
            ...
            if ($blog->status == 0) {
                if ($request->preview != 1 || !Auth::check() || Auth::user()->role_id == 3) {
                    Toastr::error(trans('blog.Blog status is not active'), 'Failed');
                    return Redirect::to('/');
                }
            }
            ...
            if (Carbon::parse($blog->authored_date_time)->gt($current_date)) {
                Toastr::error(trans('blog.Blog is not published yet'), 'Failed');
                return Redirect::to('/');
            }
            if (isModuleActive('Org')) {
                if ($blog->audience == 2) {
                    ...
                    if (!$checkBranch) {
                        Toastr::error(trans('common.Access Denied'), 'Failed');
